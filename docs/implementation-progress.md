...
Branch: feature/direct-upload-groups. Production is running zealot-s3:next-4523867e as of 2026-09-08. Historical checkpoints below describe progress at their recorded time; the final acceptance section and user scope adjustments supersede earlier pending notes.
...
- `test/s3/public_share.rb`：1 项 58 个断言，覆盖私有跳转、公开最新页、空渠道提示、锁定页不泄露应用详情或下载地址、CSRF、错误/正确密码、直接下载防绕过、R2 跳转、二维码、换密失效、管理表单与敏感字段序列化。原 S3 密码下载/iOS manifest 2 项 31 个断言通过。
- 最终镜像生产运行 `test/s3/production_public_share.rb`：公网固定短链接返回 200；公开 R2 包 SHA256 一致；密码锁定、正确密码解锁和换密撤销旧授权均通过。临时应用、版本、上传会话与对象已清理，生产恢复为 1 app、2 users、2 channels、0 releases、0 upload sessions。服务器证据为 `/tmp/zealot-production-public-share-final.log`。
- 正式镜像 `zealot-s3:next-4523867e`（amd64，`sha256:c482cbc2168a39e0d4989605cb837b9ae01bf1c18b1c1b64b8c4d78af47063e5`）healthy，schema `20260908000000`。公网 health、登录页静态资源、管理员登录、分组及存储管理页面均通过。
